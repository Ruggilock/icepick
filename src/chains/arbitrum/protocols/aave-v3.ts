...
export class AAVEv3Arbitrum {
  ...
  /**
   * Get all reserve tokens in AAVE v3 on Arbitrum
   */
  async getAllReserves(): Promise<{ symbol: string; address: string }[]> {
    const cacheKey = 'aave:arbitrum:reserves';
  ...
  async getAssetPrice(assetAddress: string): Promise<number> {
    const cacheKey = `aave:arbitrum:price:${assetAddress.toLowerCase()}`;
  ...
  async getReserveConfiguration(assetAddress: string) {
    const cacheKey = `aave:arbitrum:config:${assetAddress.toLowerCase()}`;
